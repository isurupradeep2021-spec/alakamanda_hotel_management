...
const empty = {
    ...
    rooms: { roomNumber: "", roomType: "", description: "", photoUrl: "", capacity: "", pricePerNight: "", weekendPricePerNight: "", specialRate: "", available: false, status: "" },
...
function OperationsPage({ type }) {
    ...
    const selectedEventHall = type === "events" ? eventHalls.find((hall) => hall.name === form.hallName) || null : null;
    const customerEventRows = type === "events" && isCustomerEventBookingPage ? rows.filter((row) => (row.customerEmail || "").toLowerCase() === (user?.email || "").toLowerCase()) : rows;
    const overallEventRows = type === "events" ? rows : [];
    const activeEventCount = type === "events" ? customerEventRows.filter((row) => isActiveEvent(row)).length : 0;
    const pendingConfirmationCount = type === "events" ? customerEventRows.filter((row) => (row.status || "").toUpperCase() === "INQUIRY").length : 0;
    const currentMonthEventRevenue =
        type === "events"
            ? rows.reduce((sum, row) => {
                  const status = (row.status || "").toUpperCase();
    ...
    const eventTotalPrice = type === "events" ? Number(((Number(form.pricePerGuest) || 0) * eventDurationHours + (form.packageName === "Premium" ? 10000 : 0)).toFixed(2)) : 0;
    ...
    const renderFields = () => {
        ...
        if (type === "events") {
            return (
                <div className="event-form-layout">
                    ...
                    <label>
                        Customer Email
                        <input type="email" placeholder="customer@example.com" value={form.customerEmail} onChange={(e) => setForm({ ...form, customerEmail: e.target.value })} required />
                    </label>
                    <label>
                    ...
                    {selectedEventHall && (
                        <article className="event-selection-summary">
                            ...
                            <div className="event-selection-meta">
                                <span>
                                    <i className="bi bi-house-door" /> {selectedEventHall.type}
                                </span>
                                <span>
                                    <i className="bi bi-people" /> {selectedEventHall.capacity} Guests
                                </span>
                                <span>
                                    <i className="bi bi-cash-coin" /> {formatCurrency(selectedEventHall.price_per_hour)} / hour
                                </span>
                            </div>
                        </article>
                    )}
        ...
        return (
            <div id="rooms-create-record-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "16px", width: "100%", gridColumn: "1 / -1" }}>
                <label>
                    Room Number
                    <input placeholder="Ex: 204" value={form.roomNumber} onChange={(e) => setForm({ ...form, roomNumber: e.target.value })} required />
                </label>
                <label>
                    Room Type
                ...
                <label>
                    Photo URL
                    <input type="url" placeholder="Ex: https://example.com/room-204.jpg" value={form.photoUrl} onChange={(e) => setForm({ ...form, photoUrl: e.target.value })} />
                </label>
                <label>
                    Room Description
                    <input placeholder="Ex: Sea-view room with balcony" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
                </label>
                <label>
                    Capacity
                    <input type="number" min="1" placeholder="Ex: 2" value={form.capacity} onChange={(e) => setForm({ ...form, capacity: Number(e.target.value) })} required />
                </label>
                <label>
                    Normal Price
                    <input
                ...
                <label>
                    ...
                    <input
                        ...
                        onChange={(e) => setForm({ ...form, weekendPricePerNight: Number(e.target.value) })}
                        required
                    />
                </label>
                <label>
                    Seasonal Price (Optional)
                    <input type="number" min="0" step="0.01" placeholder="Ex: 25000" value={form.specialRate} onChange={(e) => setForm({ ...form, specialRate: Number(e.target.value) })} />
                </label>
                <label>
                    Room Status
                    <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value, available: e.target.value === "AVAILABLE" })} required>
                    ...
                    <>
                        {(!isCustomerEventBookingPage || customerEventRows.length > 0) && (
                            <article className="summary-card premium-card signature-card">
                                ...
                                <p>
                                    <i className="bi bi-calendar-check" /> Upcoming and ongoing events
                                </p>
                            </article>
                        )}
                        {(!isCustomerEventBookingPage || customerEventRows.length > 0) && (
                            <article className="summary-card premium-card signature-card">
                                ...
                                <p>
                                    <i className="bi bi-clock-history" /> Inquiry requests to be confirmed
                                </p>
                            </article>
                        )}
                        {!canManageEventRecords && (
                            <article className="summary-card premium-card signature-card">
                                <div className="kpi-top">
                                    ...
                                    <span>Most Popular Hall</span>
                                </div>
                                <h3>{mostPopularHall}</h3>
                                <p>
                                    <i className="bi bi-stars" /> Most frequently booked venue overall
                                </p>
                            </article>
                        )}
                        {canManageEventRecords && (
                            <article className="summary-card premium-card signature-card">
                                <div className="kpi-top">
                                    ...
                                    <span>Event Revenue This Month</span>
                                </div>
                                <h3>{formatCurrency(currentMonthEventRevenue)}</h3>
                                <p>
                                    <i className="bi bi-bar-chart-line" /> Confirmed and completed events this month
                                </p>
                            </article>
                        )}
                    </>
                            ...
                            <article className="event-hall-card" key={hall.name}>
                                ...
                                <div className="event-hall-content">
                                    ...
                                    <div className="event-hall-specs">
                                        <span>
                                            <i className="bi bi-people" /> Capacity: {hall.capacity}
                                        </span>
                                        <span>
                                            <i className="bi bi-cash-stack" /> {formatCurrency(hall.price_per_hour)} / hour
                                        </span>
                                    </div>
                ...
                {type === "rooms" && (
                    <section className="ops-panel">
                        ...
                        <form className="crud-form premium-form" onSubmit={handleCreateRoomBooking}>
                            ...
                            <label>
                                Room Number
                                <input placeholder="Ex: 204" value={bookingForm.roomNumber} onChange={(e) => setBookingForm({ ...bookingForm, roomNumber: e.target.value })} required />
                            </label>
                            <label>
                                Check-In Date
